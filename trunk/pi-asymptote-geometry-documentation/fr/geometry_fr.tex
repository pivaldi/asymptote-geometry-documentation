...
\item \index{circle(explicit point,real)}\index{circle(pair,real)}%
  ...
  Noter que la routine \code{circle circle(pair C, real r)} est
  redéfinie afin de renvoyer un objet de type \code{circle} dont le
  centre \param{C} représente les coordonnées d'un point dans le
  repère courant \code{currentcoordsys}.

  L'exemple suivant illustre la différence entre le code
  \code{circle((0,0),R);} qui définit le cercle bleu dans le repère courant et
